...
export default function SuperGeradorCV() {
  ...
  const voltar = () => setFluxo(fluxo === 0 ? 12 : fluxo - 1);
          ...
          <div className="bg-slate-50 rounded-[2rem] p-6 border border-slate-100 mb-6">
            <div className="flex items-center gap-4 mb-4">
              <div className="w-12 h-12 rounded-2xl overflow-hidden bg-white shadow-sm flex items-center justify-center">
                {user?.imageUrl ? <img src={user.imageUrl} alt="User" /> : <User size={24} className="text-slate-300"/>}
              </div>
              <div>
        ...
        <main className="flex-1 flex flex-col md:flex-row h-[calc(100vh-72px)] overflow-hidden">
          ...
          <section className="flex-1 overflow-y-auto p-6 md:p-12 bg-white">
            <div className="max-w-xl mx-auto space-y-10 pb-20">
              ...
              {fluxo === 12 && (
                <div className="animate-in fade-in duration-700 space-y-8">
                  ...
                  <div className="grid gap-4">
                    <button onClick={() => setFluxo(1)} className="w-full p-8 bg-blue-600 rounded-[2.5rem] text-white flex items-center justify-between group shadow-xl shadow-blue-100 transition-all hover:scale-[1.01]">
                    ...
                    <div className="p-8 bg-slate-50 border-2 border-slate-100 rounded-[2.5rem] flex items-center gap-6">
                      ...
                      <button onClick={() => setFluxo(1)} className="bg-slate-900 text-white px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-600 transition-colors shadow-lg">Continuar</button>
                    </div>
                  </div>
                </div>
              )}
              ...
              {/* ETAPA 4 (IA WRITER) */}
              {fluxo === 4 || (fluxo > 1 && fluxo < 11 && fluxo !== 1) ? (
                 <div className="space-y-6 animate-in slide-in-from-right-10">
                    <div className="flex justify-between items-end">
                      <div className="space-y-1">
                        <h2 className="text-3xl font-black tracking-tighter italic text-blue-600 uppercase">Perfil Profissional</h2>
                        <p className="text-slate-400 font-bold text-sm">Descreva sua trajetória ou use nossa IA.</p>
                      </div>
                      <button 
                    ...
                    <textarea 
                      className="w-full h-64 p-8 bg-slate-50 border-2 border-transparent focus:border-blue-600 focus:bg-white rounded-[2.5rem] outline-none font-medium leading-relaxed transition-all shadow-inner" 
                      value={dados.resumo} 
                      onChange={e => update({resumo: e.target.value})} 
                      placeholder="Escreva ou deixe a IA sugerir..."
                    />
                 </div>
              ) : null}
              ...
              {fluxo === 11 && (
                <div className="animate-in zoom-in duration-500 space-y-8 text-center">
                   <div className="bg-white p-12 rounded-[3.5rem] border-4 border-blue-50 shadow-2xl relative overflow-hidden">
                      ...
                      <p className="text-slate-400 font-bold uppercase text-[10px] tracking-widest mb-10 italic">Formato ATS aprovado</p>
          ...
          <section className="hidden xl:flex flex-1 bg-slate-100 p-12 items-center justify-center overflow-hidden relative">
             ...
             <div className="w-[450px] h-[630px] bg-white shadow-2xl rounded-sm p-10 flex flex-col scale-[0.85] origin-center animate-in fade-in zoom-in duration-1000">
                ...
                <div className="flex-1 space-y-6 overflow-hidden">
                  <div className="text-[9px] text-slate-500 font-medium leading-relaxed italic">{dados.resumo || 'Sua descrição aparecerá aqui...'}</div>
